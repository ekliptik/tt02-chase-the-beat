...
class Noise(Elaboratable):
    def __init__(self, width):
        self.width = width
        self.o = Signal(unsigned(width))

    def elaborate(self, _):
        m = Module()
        # Generated by random.randint(1,2**n)
        lfsr_inits = [17611, 18444, 16948, 22913, 1470, 25977, 25111, 13098]
        for lfsr_idx in range(self.width):
            # Max length external feedback LFSR with n = 15, P(x) = x**n + x + 1
            length = 15
...
class AmaranthTop(Elaboratable):
    ...
    def elaborate(self, _):
        ...
        m.d.comb += [
            ...
            self.o.eq(Mux(self.mode, chase.o, Cat(noise.o, Const(8-noise_width))))
        ]
        return m
